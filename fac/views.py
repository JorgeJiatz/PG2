...
def facturas(request, id=None):
    ...
    if request.method == "POST":
        ...
        if not cliente:
            messages.error(request, "Seleccione cliente")
            return redirect("fac:factura_list")
...
def facturar(request):
    if request.method == "POST":
        try:
            ...
            respuestaFactura = enviarFactura(
                resultTemplate.render(infoFactura))
            ...
            if respuestaFactura:
                try:
                    ...
                    if nodo is not None:
                        ...
                        numeroAutorizacionFact = raiz.find(
                            ".//NumeroAutorizacion")
                        ...
                        infoFactura.update(
                            {"direccionFact": direccionFact.text})
                        infoFactura.update(
...
def contenido_pdf(infoFactura):
    ...
    contenido.append(
        Paragraph(f"<b>Factura NO.</b> {infoFactura['numero']}", estilo3))
    contenido.append(
        Paragraph(
            f"<b>Referencia NO.</b> {infoFactura['referencia']}", estilo4)
    )
    contenido.append(
        Paragraph(f"<b>Serie NO.</b> {infoFactura['serie']}", estilo4))
    contenido.append(
        Paragraph(f"<b>Fecha.</b> {infoFactura['fecha']}", estilo4))
    ...
    if infoFactura['estado'] is not None and infoFactura['estado'] == 'A':
        contenido.append(Paragraph("Factura Anulada", estilo8))
    contenido.append(Paragraph("=" * 37, estilo))
    contenido.append(Paragraph(f"<b>NIT.</b> {infoFactura['nit']}", estilo4))
    contenido.append(
        Paragraph(f"<b>Nombre.</b> {infoFactura['nombreFact']}", estilo4))
    contenido.append(
...
def data_fact(infoFactura):
    ...
    data.append(
        [Paragraph("<b>Nombre :</b> GUATEFACTURAS SOCIEDAD ANONIMA", estilo7), ""])
...
def anularFactura(request):
    ...
    id = request.POST.get('id')
    try:
        fact_enc = FacturaEnc.objects.get(id=id)
        serie = fact_enc.serie_factura
        numero = fact_enc.numero_factura
        nit = fact_enc.nit_factura
        formato = "%Y%m%d"
        fecha = date.today().strftime(formato)

        resultado = enviarAnulacion(serie, numero, nit, fecha, motivo)
        raiz = ET.fromstring(str(resultado.strip()))
        nodo = raiz.find(".//ESTADO")

        if nodo is not None:
            detalle = FacturaDet.objects.filter(factura=id)

            for item in detalle:
                infoProducto = Producto.objects.get(
                    codigo=item.producto.codigo)
                infoProducto.existencia += item.cantidad
                infoProducto.save()

            fact_enc.estado_fel = 'A'
            fact_enc.save()

            response = {"flag": True, "mensaje": resultado}
            return JsonResponse(response)
        else:
            response = {"flag": False, "mensaje": resultado}
            return JsonResponse(response)
    except Exception as ex:
        response = {"flag": False, "mensaje": str(ex)}
        return JsonResponse(response)
...
def enviarAnulacion(serie, numero, nit_comprador, fecha, motivo):
    usuario = "WSFEL_81089481"
    password = "WSFEL_LVSSJVIQI"
    nit = 81089481
    ...
    respuesta = clinte.service.anulaDocumento(
        usuario, password, nit, serie, numero, nit_comprador, fecha, motivo
    )
    return respuesta
